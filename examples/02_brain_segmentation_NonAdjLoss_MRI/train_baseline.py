    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        try:
            if semi_loader is None:
                train(train_loader, model, optimizer, nll_loss,
        ...
        validate(val_loader, model, nll_loss, dice_weight, epoch, log_plot)
        ...
        log_plot.log('learning_rates.csv', '{:d};{:.7f};{:.3f}'.format(epoch, lr, dice_weight))
        update_figures(log_plot)

        # save checkpoint
        if train_nan_flag is False:
            save_checkpoint({
                'epoch': epoch,
...
def train(train_loader, model, optimizer, nll_loss, dice_weight, epoch, logger):
...
def validate(val_loader, model, nll_loss, dice_weight, epoch, logger):
    logger.clear_metrics()
